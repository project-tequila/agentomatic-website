...
function c(iso2: string, dial: string, label: string): CountryCode {
  const id = iso2.toLowerCase();
  return { id, code: dial, label, flag: flagEmoji(iso2) };
}

/** US first, then alphabetical by country name. */
const countryEntries: Array<[string, string, string]> = [
  ["US", "+1", "United States"],
  ["AF", "+93", "Afghanistan"],
  ["AL", "+355", "Albania"],
  ["DZ", "+213", "Algeria"],
...
export const countryCodes: CountryCode[] = countryEntries.map(([iso, dial, label]) => c(iso, dial, label));

export function countryById(id: string) {
  return countryCodes.find((entry) => entry.id === id) ?? countryCodes[0]!;
}

export function countryFlagUrl(id: string) {
