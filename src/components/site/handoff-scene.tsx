  ...
  return (
    <g transform={`translate(${x} ${y})`} opacity={intensity}>
      <g
        ref={setMagRef}
        transform={`translate(${magX} ${magY})`}
        className="concurrent-scene__phone-magnetic handoff-scene__beacon handoff-scene__glyph-motion"
      >
...
export function HandoffScene({ story, opacity: sceneOpacity }: HandoffSceneProps) {
  ...
  const progress = featureBandSceneProgress(story, "handoff");

  const { offsets: flowOffsets, setGroupRef: setFlowRef } = useStoryFlowMagnetic(
    ["callerOrb", "callerHuman", "summary"],
    { disabled: !!reduceMotion },
  );
  const callerOrbMag = flowOffsets.callerOrb ?? { x: 0, y: 0 };
  const callerHumanMag = flowOffsets.callerHuman ?? { x: 0, y: 0 };
  const summaryMag = flowOffsets.summary ?? { x: 0, y: 0 };

  const { offsets: iconOffsets, setGroupRef: setIconRef } = useSvgGroupMagnetic(
    ["caller", "human", "summaryCard", "beacon"],
    {
      strength: 0.4,
      maxDisplacement: 12,
      radiusFactor: 1.15,
  ...
  const callerMag = iconOffsets.caller ?? { x: 0, y: 0 };
  const humanMag = iconOffsets.human ?? { x: 0, y: 0 };
  const summaryCardMag = iconOffsets.summaryCard ?? { x: 0, y: 0 };
  const beaconMag = iconOffsets.beacon ?? { x: 0, y: 0 };
  ...
  return (
    ...
    >
      ...
      <g opacity={callerToHuman}>
        <g
          ref={(node) => setFlowRef("callerHuman", node)}
          transform={`translate(${callerHumanMag.x} ${callerHumanMag.y})`}
          className="story-flow-magnetic"
        >
          <path
            d={callerHumanPath}
      ...
      <CallerPhone
        ...
        magX={callerMag.x}
        magY={callerMag.y}
        setMagRef={(node) => setIconRef("caller", node)}
      />

      <g opacity={transferIntensity}>
        <g
          ref={(node) => setFlowRef("summary", node)}
          transform={`translate(${summaryMag.x} ${summaryMag.y})`}
          className="story-flow-magnetic"
        >
          <path
            d={summaryPath}
      ...
      <TransferBeacon
        x={transferMidX}
        y={transferMidY}
        intensity={transferPeak * 0.92}
        reduceMotion={!!reduceMotion}
        magX={beaconMag.x}
        magY={beaconMag.y}
        setMagRef={(node) => setIconRef("beacon", node)}
      />
      ...
      <HumanAgent
        ...
        magX={humanMag.x}
        magY={humanMag.y}
        setMagRef={(node) => setIconRef("human", node)}
      />
      ...
      <CallSummaryCard
        ...
        glow={summary.glow}
        satelliteScale={satelliteScale}
        magX={summaryCardMag.x}
        magY={summaryCardMag.y}
        setMagRef={(node) => setIconRef("summaryCard", node)}
      />
    </svg>
  );
}
